...
freevars :: Term -> String -> [String]
freevars lterm@(Var x) s = if s == x then [] else [x]
freevars lterm@(Application x y) s = (freevars x s)++(freevars y s)
freevars lterm@(Abstraction str x) s = (freevars x s)

sub :: Term -> Int -> String -> Term
sub lterm@(Var x) c str = (Var str)
sub lterm@(Application x y) c str = (Application (sub x c str) (sub y c str))
sub lterm@(Abstraction s x@(Abstraction s1 x1)) c str = case [(chr c)]==s1 of
                                                True -> (sub lterm (succ c) [(chr (succ c))])
                                                False -> (Abstraction [(chr c)] (sub x c [(chr c)]))

alpha :: Term -> Term -> Term
alpha lterm@(Application x y) rterm= (Application (alpha x rterm) (alpha y rterm))
alpha lterm@(Abstraction s x) rterm@(Abstraction s1 y) = case s1 `elem` (freevars lterm s) of
                                                          True -> (sub lterm 97 "a")
                                                          False -> lterm
